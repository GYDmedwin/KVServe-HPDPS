...
class KVHadamardQuantOp:
    ...
    # ===================================================================
    def _channel_compress_fwht(self, data_fp16, signs_fp16, max_value_vec):
    ...
    def _token_compress_fwht(self, data_fp16, signs_fp16, max_value_vec):
        ...
        mv_id = id(max_value_vec)
        if mv_id not in self._tok_gmap_cache:
            ...
            self._tok_gmap_cache[mv_id] = gmap_t

        group_map = self._tok_gmap_cache[mv_id]
        ...
        # Pack metadata into a compact per_group_meta dict to align with legacy size
        per_group_meta = {}
        unique_mvs = torch.unique(max_value_vec)
        for mv in unique_mvs:
            mv_val = mv.item()
            head_indices = (max_value_vec == mv).nonzero(as_tuple=True)[0]
            first_head = head_indices[0].item()
            # Extract unique scale & min columns for this group and format to 3D [rows, 1, 1]
            min_val_group = g_min_out[:, first_head:first_head+1].unsqueeze(-1)
            scale_group = g_scale_out[:, first_head:first_head+1].unsqueeze(-1)
            
            per_group_meta[mv_val] = {
                "head_idx": head_indices,
                "min_val": min_val_group,
                "quant_scale": scale_group,
            }

        meta = {
            "per_group_meta": per_group_meta
        }
        return q_out, meta
    ...
    def compress_v3(
        ...
        q_keys, k_meta = self._channel_compress_fwht(keys_bf16, signs, key_max_vals)
        q_keys = q_keys.reshape(num_blocks, block_size, num_heads, head_dim)

        # ─── Value：FWHT + token reduce + quantize v2（2 kernels，零 head-split）───
        vals_bf16 = values.reshape(rows, num_heads, head_dim)
        q_vals, v_meta = self._token_compress_fwht(vals_bf16, signs, val_max_vals)
        q_vals = q_vals.reshape(num_blocks, block_size, num_heads, head_dim)

        out = torch.stack([q_keys, q_vals], dim=0)
        metadata = {
            "key_meta": k_meta,
            "value_meta": v_meta,
